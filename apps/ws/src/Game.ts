...
export class Game {
    ...
    async updateSecondPlayer(player2UserId: string) {
        ...
        SocketManager.getInstance().broadcast(this.gameId, JSON.stringify({
            ...
            payload: {
                gameId: this.gameId,
                whitePlayer: { name: users.find((user: User) => user.id === this.player1UserId)?.name, id: this.player1UserId },
                blackPlayer: { name: users.find((user: User) => user.id === this.player2UserId)?.name, id: this.player2UserId },
                fen: this.board.fen(),
                moves: []
            }
        }));
